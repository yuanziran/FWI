...
int main(int argc, char**argv)
{
	...
	vp_model = FWIAllocateMemory(XDIM*ZDIM, 0.0f);
    ...
    float* vs_model = FWIAllocateMemory(XDIM*ZDIM, 0.0f);
	if(!vs_name.empty())
    {
        raw_vs_model=FWIAllocateMemory(NX*NZ,0.0f);
        Read2DBin(vs_name.c_str(),raw_vs_model,NX,NZ);
        Expand2D(vs_model,raw_vs_model,XDIM,ZDIM,NX,NZ,OFFSET);
    }
    else
    {
        float scalar=1.0f/1.73f;
        for(int i=0;i<(XDIM*ZDIM);++i)
        {
    ...
    float* vz_dx_phi1 = FWIAllocateMemory(ABC_LAYERS*ZLEN,0.0f);
	float* vz_dx_phi2 = FWIAllocateMemory(ABC_LAYERS*ZLEN, 0.0f);
	float* sigma_x_dx_phi1 = FWIAllocateMemory(ABC_LAYERS*ZLEN, 0.0f);
	float* sigma_x_dx_phi2 = FWIAllocateMemory(ABC_LAYERS*ZLEN, 0.0f);
    float* tau_y_dx_phi1=FWIAllocateMemory(ABC_LAYERS*ZLEN, 0.0f);
    float* tau_y_dx_phi2=FWIAllocateMemory(ABC_LAYERS*ZLEN, 0.0f);

    float* vx_dz_phi1 = FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
	float* vx_dz_phi2 = FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
	float* vz_dz_phi1 = FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
	float* vz_dz_phi2 = FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
	float* sigma_z_dz_phi1 = FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
	float* sigma_z_dz_phi2 = FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
	float* tau_y_dz_phi1= FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
    float* tau_y_dz_phi2= FWIAllocateMemory(ABC_LAYERS*XLEN, 0.0f);
    //*********************检波器测线*********************
	//和地表平行的测线
	int XLINE_ZPOS = 0;
	int XLINE_XBEGIN = 0;
    ...
    int RECORD_XLEN=GetLen(XLINE_XBEGIN,XLINE_XINTERVAL,XLINE_XEND);
	//检波器缓冲区
	//垂直的测线
    ...
    int RECORD_ZLEN=GetLen(ZLINE_ZBEGIN,ZLINE_ZINTERVAL,ZLINE_ZEND);
	//检波器缓冲区
	float* sigma_xx_xline_geo = FWIAllocateMemory(NT*RECORD_XLEN,0.0f);
    float* sigma_zz_xline_geo = FWIAllocateMemory(NT*RECORD_XLEN,0.0f);
    float* tau_y_xline_geo = FWIAllocateMemory(NT*RECORD_XLEN,0.0f);
	float* vx_xline_geo = FWIAllocateMemory(NT*RECORD_XLEN, 0.0f);
	float* vz_xline_geo = FWIAllocateMemory(NT*RECORD_XLEN, 0.0f);

	float* sigma_xx_zline_geo = FWIAllocateMemory(NT*RECORD_ZLEN, 0.0f);
    float* sigma_zz_zline_geo = FWIAllocateMemory(NT*RECORD_ZLEN, 0.0f);
    float* tau_y_zline_geo = FWIAllocateMemory(NT*RECORD_ZLEN, 0.0f);
	float* vx_zline_geo = FWIAllocateMemory(NT*RECORD_ZLEN, 0.0f);
	float* vz_zline_geo = FWIAllocateMemory(NT*RECORD_ZLEN, 0.0f);
	//申请波场
	float* sigma_xx = FWIAllocateMemory(XDIM*ZDIM,0.0f);
    float* sigma_zz = FWIAllocateMemory(XDIM*ZDIM,0.0f);
    float* tau_y = FWIAllocateMemory(XDIM*ZDIM,0.0f);
	...
	for (int t = 0; t < NT; ++t)
	{
        ...
        ElSgStressToVelocityCPML2D(vx,vz,sigma_xx,sigma_zz,tau_y,dense_model,XDIM,ZDIM,XLEN,ZLEN,CDO_ORDER,sg_cdo_coeff,
		ABC_LAYERS, x1_cpml_alpha, x1_cpml_beta, x2_cpml_alpha, x2_cpml_beta,
			z1_cpml_alpha, z1_cpml_beta, z2_cpml_alpha, z2_cpml_beta,
            sigma_x_dx_phi1,sigma_x_dx_phi2,tau_y_dx_phi1,tau_y_dx_phi2,
			sigma_z_dz_phi1,sigma_z_dz_phi2,tau_y_dz_phi1,tau_y_dz_phi2,
            DX,DZ,DT);
		ElSgVelocityToStressCPML2D(sigma_xx,sigma_zz,tau_y,vx,vz,
        dense_model,vp_model,vs_model,XDIM,ZDIM,XLEN,ZLEN,CDO_ORDER,
			sg_cdo_coeff,ABC_LAYERS,x1_cpml_alpha,x1_cpml_beta,x2_cpml_alpha,x2_cpml_beta,
			z1_cpml_alpha,z1_cpml_beta,z2_cpml_alpha,z2_cpml_beta,
            vx_dx_phi1,vx_dx_phi2,vz_dx_phi1,vz_dx_phi2,
			vx_dz_phi1,vx_dz_phi2,vz_dz_phi1,vz_dz_phi2,
            DX,DZ,DT);
		//添加检波器记录的
		for (int i = 0; i < RECORD_XLEN; ++i)
		{
			const size_t record_to_pos = t * RECORD_XLEN + i;
			const size_t record_from_pos = (i*XLINE_XINTERVAL + OFFSET+XLINE_XBEGIN)*ZDIM + XLINE_ZPOS+OFFSET;
            ...
            vz_xline_geo[record_to_pos]=vx[record_from_pos];
		}
		for (int j = 0; j < RECORD_ZLEN; ++j)
		{
			const size_t record_to_pos = t * RECORD_ZLEN + j;
			const size_t record_from_pos = (ZLINE_XPOS + OFFSET)*ZDIM + j*ZLINE_ZINTERVAL + OFFSET+ZLINE_ZBEGIN;
			sigma_xx_zline_geo[record_to_pos] = sigma_xx[record_from_pos];
            sigma_zz_zline_geo[record_to_pos] = sigma_zz[record_from_pos];
            tau_y_zline_geo[record_to_pos] = sigma_xx[record_from_pos];
            vx_zline_geo[record_to_pos]=vx[record_from_pos];
            vz_zline_geo[record_to_pos]=vx[record_from_pos];
		}
	}
	TIME_END("El2D")
    ...
    Write2DBinEx(XLINE_NAME.c_str(), "-vx-", vx_xline_geo, NT, RECORD_XLEN);
    Write2DBinEx(XLINE_NAME.c_str(), "-vz-", vz_xline_geo, NT, RECORD_XLEN);

    Write2DBinEx(ZLINE_NAME.c_str(),"-sigma-xx-",sigma_xx_zline_geo,NT,RECORD_ZLEN);
	Write2DBinEx(ZLINE_NAME.c_str(), "-sigma-zz-", sigma_zz_zline_geo, NT, RECORD_ZLEN);
	Write2DBinEx(ZLINE_NAME.c_str(), "-tau-y-", tau_y_zline_geo, NT, RECORD_ZLEN);
    Write2DBinEx(ZLINE_NAME.c_str(), "-vx-", vx_zline_geo, NT, RECORD_ZLEN);
    Write2DBinEx(ZLINE_NAME.c_str(), "-vz-", vz_zline_geo, NT, RECORD_ZLEN);
	...
	DebugFreeMemory(&vx);
	DebugFreeMemory(&vz);
	DebugFreeMemory(&sigma_xx_xline_geo);
    DebugFreeMemory(&sigma_zz_xline_geo);
    DebugFreeMemory(&tau_y_xline_geo);
	DebugFreeMemory(&vx_xline_geo);
	DebugFreeMemory(&vz_xline_geo);
    DebugFreeMemory(&sigma_xx_zline_geo);
    DebugFreeMemory(&sigma_zz_zline_geo);
    DebugFreeMemory(&tau_y_zline_geo);
